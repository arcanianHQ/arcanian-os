...
| clients/wellis | asana | C:Wellis Transition | 1213304520507935 |
| clients/diego | todoist | TBD | TBD |
| clients/mancsbazis | todoist | TBD | TBD |
| clients/deluxe | todoist | TBD | TBD |
...
2. If not found, check `$USER` env var → map: `endre` = László
3. If still unknown, ask: "Who is running this session? (László / Éva / Dóra)"
...
```
SYNC PLAN — clients/wellis ↔ Asana
...
Domain: buenospa.com
FND: FND-009, FND-011
SOP: measurement-audit/02-phase-1-browser
Goal: Q1-mancsbazis-compliant
Client: mancsbazis
Local task: #9
...
| Domain | `labels` | Create label per domain: "buenospa.com", "wellis.hu", etc. |
...
```json
{
  ...
  "description": "Layer: L5\nDomain: buenospa.com\nSOP: 05-CAMPAIGN-MANAGEMENT\nClient: wellis\nLocal task: #115\nWaiting on: Viktor + Intren — keret jóváhagyás\nWaiting since: 2026-03-26\nFollow-up: 2026-03-28 — László emlékeztető Gábornak\nDepends on: #113",
  "priority": "p1",
  "labels": ["waiting", "impact-unlock", "buenospa.com"],
...
```json
{
  ...
  "description": "Layer: L5\nDomain: buenospa.com, wellisspa.com, wellis.hu, wellis.fr, wellis.nl, wellis.com, wellisfactorystore.com\nSOP: 07-VENDOR-ACCESS\nClient: wellis\nLocal task: #138",
  "priority": "p2",
...
- [ ] #9 Add consent gating to all Google tags
  - @next @computer
  - P0 | Owner: László | Effort: 2h | Impact: hygiene
...
**Log to CAPTAINS_LOG:**
```
## 2026-03-24 — Task sync: mancsbazis ↔ Todoist
...
## Single Task Auto-Push (ALWAYS-ON)

When a SINGLE task is created or updated in TASKS.md:
1. If the task has Layer + Priority + Owner fields → auto-push to Todoist immediately
2. Determine the correct `projectId` from TODOIST_ROUTING_MAP.md (by client slug or keyword)
3. Push the task via Todoist MCP
4. Show one-line confirmation: "Synced #{N} to Todoist (project: {name})"
5. Update the task in TASKS.md with `ext:` and `synced:` fields

**No confirmation prompt needed for single tasks.** The TASKS.md write already happened — the human approved the task by creating it.

**For BULK operations (5+ tasks):** keep existing behavior — show sync plan, confirm, batch with 10/call + 3s delays.

**Threshold:** 1-4 tasks = auto-push each. 5+ tasks = show plan and confirm.
